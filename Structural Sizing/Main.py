...
#TODO CHANGE LEG TO SOLID
#TODO FIX TWIST ANGLE (DEG2RAD?)
#------------------------------------------------------
...
R_in_Tail = 0.01
R_out_Tail = 0.01+1/1000

Tail_loading_horizontal_Distributed = 30 #Newtons per meter
Tail_loading_Vertical_Distributed = 20 #Newtons per meter
...
F_prop = 100 #Newtons
T_prop = 30 #Newtons /Meter
Main_Engine_Thrust = F_prop
Main_Engine_Torque = T_prop

Fuselage_Sec1_mass = 1+0.5+0.4 #kg
Fuselage_Sec2_mass = 5 +0.7 #kg

Fuselage_Sec1_Load = (Fuselage_Sec1_mass*9.81)/Fuselage_length_sec1  #N*m SENSORS +C OMSS +M AIN MOTOR (distributed load)
Fuselage_sec2_load = (Fuselage_Sec2_mass*9.81)/Fuselage_length_sec2 #N*m BATTERY+PARACHUTE
...
WingBox_length = 1.5
R_out_WingBox = 0.12*MAC
R_in_WingBox = 0.12*MAC-1/1000
...
while Big_owie_WingBox:
    ...
    WingBox_Deflection_angle1 = Tip_Deflection_angle(F=(2*F_Vtol-Tail_Effective_Length*Tail_loading_Vertical_Distributed + Wing_Total_Lift),L=WingBox_length,E=Material_WingBox.E,I=WingBox_I)
    WingBox_Deflection_angle2 = Tip_Deflection_angle(F=(2*F_Vtol-Tail_Effective_Length*Tail_loading_Vertical_Distributed),L=WingBox_length,E=Material_WingBox.E,I=WingBox_I)
    WingBox_Deflection1 = Tip_Deflection(F=(2*F_Vtol-Tail_Effective_Length*Tail_loading_Vertical_Distributed + Wing_Total_Lift),L=WingBox_length,E=Material_WingBox.E,I=WingBox_I)
    WingBox_Deflection2 = Tip_Deflection(F=(2*F_Vtol-Tail_Effective_Length*Tail_loading_Vertical_Distributed),L=WingBox_length,E=Material_WingBox.E,I=WingBox_I)
    WingBox_Total_Deflection = WingBox_Deflection1+WingBox_Deflection2+WingBox_Deflection_angle2*(WingBox_length-Vtol_Location)
   
    WingBox_Twist_Angle = Twist(T = WingBox_Torsion_Shear_Z,  L= WingBox_length, G=Material_WingBox.G, J=WingBox_J)
    ...
    print("The Twist Angle:", np.rad2deg(WingBox_Twist_Angle), "And the allowable angle is:", 1)
    print("The Wingbox Thickness:",R_out_WingBox, R_in_WingBox,R_out_WingBox - R_in_WingBox)


    if Von_Mises_Wingbox_Stress < Yield_Stress_WingBox and Von_Mises_Wingbox_Stress < WingBox_Buckle and Yield_shear_WingBox > Von_Mises_Wingbox_Shear and WingBox_Total_Deflection < 0.1*WingBox_length and np.rad2deg(WingBox_Twist_Angle) < 1:
