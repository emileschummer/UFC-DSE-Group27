...
def calculate_power_UFC_MMA_2(incline, V, rho, inputs):
    ...
    Tvertical = np.cos(incline)*inputs[0]/inputs[12]
    Thorizontal = (D_parasite + np.sin(incline)*inputs[0])/inputs[12]
    T = (Tvertical**2 + Thorizontal**2)**0.5
    A_prop = inputs[6]/inputs[12]
    #Solve for vi
    ...
    P_induced = vi * T * inputs[12]
    P_parasite = D_parasite * V #* numberengine_MMA2
    P_profile = (inputs[7]*inputs[8]*rho*inputs[9]*inputs[10]**3*inputs[5]**4*(1+3*(V/(inputs[10]*inputs[5]))**2))/8 * inputs[12]
