...
def calculate_power_UFC_MMA_1(incline,V,rho, inputs, max_iter=100, tol=1e-3):
    D_profile = 0.5*rho*inputs[3]*inputs[2]*V**2
    Tvertical = np.cos(incline)*inputs[0]
    Thorizontal = D_profile + np.sin(incline)*inputs[0]
    T = (Tvertical**2 + Thorizontal**2)**0.5
    alpha_T= np.cos(Tvertical/T)
    #T_hover= inputs[0]
    V_ind_hover= (T/(2*rho*inputs[4]))**0.5#(T_hover/(2*rho*inputs[4]))**0.5
    ...
    P_induced = vi *T#Tvertical 
    P_parasite = 0# Thorizontal * V
