...
@pytest.fixture
def mock_wing_geom():
    """Creates a mock wing geometry with 5 cross-sections."""
...
def mock_lift_distribution():
    """Provides a sample lift distribution for different alphas."""
    return {
        "alpha": [0, 5, 10],
        "CLs": [
...
def test_AF_1(mock_plt, mock_wing_geom, mock_lift_distribution):
    ...
    """
    Tests if the function correctly calculates the normalized spanwise locations
    and returns the correct data structure for a specific alpha.
    """
    alpha_to_test = 5
    ...
    distribution = load_distribution_halfspan(
        ...
        alpha=alpha_to_test,
        half_span=half_span,
        plot=False  # Disable plotting for the test
    )

    expected_div = np.array([0.125, 0.375, 0.625, 0.875])

    # Expected CLs are the ones for alpha=5 from the mock distribution
    ...
    # Check that plotting functions were not called since plot=False
