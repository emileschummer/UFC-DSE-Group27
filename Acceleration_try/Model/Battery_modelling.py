...
def Battery_Model(output_folder, V_vert_prop=5, W=250, D_rest=50, CLmax=2.2, S_wing=1.5, piAe=20.41, CD0_wing=0.0264, numberengines_vertical=4, numberengines_horizontal=1, propeller_wake_efficiency=0.8, number_relay_stations=1, UAV_off_for_recharge_time_min =15,battery_recharge_time_min =5,PL_power = 189,  show=False, L_fus=1.2, L_n=0.2, L_c=0.6, d=0.25, L_blade=0.7366, L_stab=0.6):
    ...
    for race_name, race_data in races.items():
        ...
        time_plot, distance_plot, power_plot, speed_plot, gradient_plot, acceleration_plot, pitch_rate_plot, rho_plot, battery_energy_plot, altitude_plot = simulate_1_battery(df_vertical,df_horizontal,race_data, calculate_power, W, V_vert_prop, CLmax, S_wing, CD0_wing, piAe, numberengines_vertical,numberengines_horizontal, propeller_wake_efficiency,PL_power, L_fus, L_n, L_c, d, L_blade, L_stab)
        ...
        while n_switches < number_relay_stations and idx < len(time_plot):
            ...
            for j in range(go_charge_idx, reached_RS_idx + 1):
                if new_distance[j-1] < RS_location:
                    ...
                    P = calculate_power(df_vertical,df_horizontal,gradient,velocity,rho, acceleration, pitch_rate, W, V_vert_prop, CLmax, S_wing, CD0_wing, piAe, numberengines_vertical,numberengines_horizontal, propeller_wake_efficiency,L_fus=L_fus, L_n=L_n, L_c=L_c, d=d, L_blade=L_blade, L_stab=L_stab)
...
def simulate_1_battery(df_vertical,df_horizontal,race_data, calculate_power, W, V_vert_prop, CLmax, S_wing, CD0_wing, piAe, numberengines_vertical,numberengines_horizontal, propeller_wake_efficiency,PL_power, L_fus, L_n, L_c, d, L_blade, L_stab):
    # Prepare arrays
    ...
    for index, row in race_data.iterrows():
            ...
            P = calculate_power(df_vertical,df_horizontal,grade_smooth,velocity_smooth,rho, acceleration,pitch_rate, W, V_vert_prop, CLmax, S_wing, CD0_wing, piAe, numberengines_vertical,numberengines_horizontal, propeller_wake_efficiency, L_fus, L_n, L_c, d, L_blade, L_stab)
            P+= PL_power  # Add power for payload
